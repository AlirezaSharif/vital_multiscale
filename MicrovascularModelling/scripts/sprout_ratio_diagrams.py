...
def main():
    ...
    output_dir = "./diagrams/sprout_ratio"
    ...
    string = f"./outputs/SPROUT_RATIO"
    config.config_access["RUN_PARAMETERS"]["output_path"] = string

    parent_folder = Path("./outputs/SPROUT_RATIO")
    folders = sorted(
    [item for item in parent_folder.iterdir() if item.is_dir() and item.name.startswith("ratio_")],
    key=lambda f: float(f.name.split("_")[1]))
    
    # If you only want the names, not the full Path objects:
    ratio_strings = [folder.name for folder in folders]
    config.logger.log(ratio_strings)
    ...
    for strategy_index, string in enumerate(ratio_strings):
        vasc_data_by_gen = {}
        config.config_access["RUN_PARAMETERS"]["test_name"] = string
        _, _, _, output_path, test_name = config.parse_run()  # type: ignore
        filepath = os_path_join(output_path, test_name)
        case = config.growth_case  # type: ignore
    ...
    def plot_attributes_over_generations(data_by_strat, output_dir):
        ...
        for attribute in attributes:
            ...
            for (strategy_i, (strat_key, gen_dataset)) in enumerate(data_by_strat.items()):
                ...
                plt.plot(gen_x, values, label=f"Ratio {(strat_key+1)/10}", marker='o', color=colors[strategy_i])
        ...
        #         values = [gen_dataset[gen][attribute] for gen in generations]
        #         plt.plot(
        #             generations, values, label=f"Ratio {(strat_key+1)/10}",
    ...
    def plot_attributes_surface(data_by_strat, output_dir):
        ...
        strategy_ratios = np.array([(key + 1) / 10 for key in strategy_keys])  # use actual ratio values
        ...
        for attribute in attributes:
            ...
            ax.set_xlabel("Flow-Volume Ratio")
            ...
            num_ticks = 11  # or any number you prefer
