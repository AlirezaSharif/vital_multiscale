...
def main():
    ...
    config.config_access["RUN_PARAMETERS"]["output_path"] = f"./outputs/P_STRAT_{strategy_num}"
    config.config_access["GROWTH_PARAMETERS"]["sprouting_strategy"] = 3
    config.config_access["GROWTH_PARAMETERS"]["pressure_strategy"] = strategy_num
    ...
    for loops in range(num_loops):
        for i in range(max_iter):
            ...
            solver.iterative_solve_vegf(alpha=1)
            mat_handler.save_tissue(age,vegf=True)
            new_pressure = growth_handler.run_pressure_control_mechanism(ratio,strategy_num)
