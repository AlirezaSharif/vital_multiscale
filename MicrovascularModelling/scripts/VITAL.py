...
def main():
    ...
    pressure_strategy_num = 0#1 # Pressure control strategy to use
    ...
    def connect_artery_vein_trees_merge(file1, file2, inlet1_coord, inlet2_coord, 
                                terminal_array="isTerminal", merge_tol=1e-8, debug=True):
        """
        Connects Artery (Flow: Inlet->Capillary) to Vein (Flow: Capillary->Outlet).
        
        Args:
            merge_tol (float): Distance threshold. If terminals are closer than this, 
                            they are merged into a single node instead of adding a bridge segment.
        """
        ...
        # --- NEW STEP: Handle Merges First ---
        # If nodes are super close, create ONE node for BOTH keys immediately
        merged_count = 0
        for key1, key2 in bridge_pairs:
            p1 = data1["points"][key1[1]]
            p2 = data2["points"][key2[1]]
            ...
            if dist < merge_tol:
                ...
                # Create a standard passing node (not inlet/outlet)
                new_node = my_tree.add_node(midpoint, current_uid)
                
                # Map BOTH keys to this single node object
                node_obj_map[key1] = new_node
                node_obj_map[key2] = new_node
                
                global_id_counter[0] += 1
                merged_count += 1
                
        print(f"Merged {merged_count} close terminal pairs into single nodes.")
        ...
        def add_nodes_from_file(file_idx, data):
            ...
            for i in range(len(points)):
                ...
                # SKIP if already created during the Merge Step
                if key in node_obj_map:
                    continue
                ...
                if key == global_inlet_key:
                    ...
                    # This is a bridged node that was NOT close enough to merge
                    # So we create it as a standard node (connector)
        ...
        for u, v in nx.bfs_edges(G_art, source=inlet_id_1):
            ...
            r = 3e-6
            print(f"Adding Artery Segment: {nu} -> {nv}")
        ...
        for u, v in nx.bfs_edges(G_vein, source=inlet_id_2):
            ...
            r = 3e-6
            # Flow goes v -> u (upstream to downstream/root)
            print(f"Adding vein Segment: {nv} -> {nu}")
            my_tree.add_segment(nv, nu, seg_counter, radius=r)
            seg_counter += 1
                
        # --- BRIDGE SEGMENTS ---
        for key1, key2 in bridge_pairs:
            nu = node_obj_map[key1] # Artery Terminal
            nv = node_obj_map[key2] # Vein Terminal
            
            # LOGIC CHANGE: 
            # If nu and nv are the exact same object (because we merged them),
            # do NOT add a bridge segment.
            if nu == nv:
                continue
            ...
            # r_bridge = (r1+r2)/2.0
            r_bridge = 3e-6
            print(f"Adding bridge Segment: {nu} -> {nv}")
