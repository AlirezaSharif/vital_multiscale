...
def main():
    ...
    config.config_access["RUN_PARAMETERS"]["output_path"] = f"./outputs/SPROUT_SPRING_{spring_level}"
    config.config_access["GROWTH_PARAMETERS"]["pressure_strategy"] = 2
    config.config_access["GROWTH_PARAMETERS"]["k_x"] = spring_level * 0.003
    ...
    for loops in range(num_loops):
        for i in range(max_iter):
            ...
            solver.iterative_solve_vegf(alpha=1)
            mat_handler.save_tissue(age,vegf=True)
            new_pressure = growth_handler.run_pressure_control_mechanism(ratio,2)
        ...
        while adjustment_made and i < 8:
            ...
            if adjustment_made:
                new_pressure = growth_handler.set_inlet_pressure_for_flow_volume_ratio(ratio)
            age = growth_handler.age
        ...
        norms, convergence_rates = solver.iterative_solve_oxygen(tolerance=1e-8, max_iterations=500,alpha=0.85,momentum=0.95,very_verbose=False)
        solver.iterative_solve_vegf(alpha=1)

        tissue = tissue_handler.current_tissue()
        tissue.sample_n_subdomains(30,21.6,my_tree,loops,True,False,growth_handler)
        tissue.sample_n_subdomains(30,21.6,my_tree,loops,False,False,growth_handler)
